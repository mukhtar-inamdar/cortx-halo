...
from backend.rest_server.file_transfer import FileRef, FileCache
# from csm.common.errors import CsmInternalError
from backend.errors import CsmInternalError
import os
...
class CsmView(web.View):
    ...
    def get_roles(cls, handler, method):
        ...
        and from the whole view.
        """
        view_roles = CsmAuth.get_roles(handler)
        method_handler = cls._get_method_handler(handler, method)
        if method_handler is not None:
            method_roles = CsmAuth.get_roles(method_handler)
            # TODO: Merge view and method permissions?
            # permissions = view_permissions | method_permissions
            roles = method_roles
        else:
