...
from backend.conf import ConfSection, DebugConf
from cortx.utils.log import Log
# from cortx.utils.product_features import unsupported_features
# from csm.common.payload import Json
# from csm.core.blogic import const
from backend import const
from backend.errors import (CsmError, CsmNotFoundError, CsmPermissionDenied,
...
from backend.rest_server.file_transfer import DownloadFileEntity
# from csm.core.controllers.view import CsmView, CsmAuth, CsmHttpException
from backend.rest_server.controllers.view import CsmView, CsmAuth, Response, CsmHttpException
# from csm.core.controllers.routes import CsmRoutes
from backend.rest_server.controllers.routes import CsmRoutes, ApiRoutes
import re
from cortx.utils.errors import DataAccessError
from marshmallow import ValidationError, fields
# from csm.core.controllers.validators import ValidateSchema
from backend.rest_server.controllers.validators import ValidateSchema
...
class CsmRestApi(ABC): #  CsmApi, ABC):
    ...
    @classmethod
    async def _get_roles(cls, request):
        handler = await CsmRestApi._resolve_handler(request)
        return CsmView.get_roles(handler, request.method)
    ...
    @staticmethod
    async def _validate_bearer(user_manager: Any, session_id: str):
        """
    ...
    async def session_middleware(request, handler):
        ...
        try:
            ...
            CsmRestApi._unauthorized(e.error())
    ...
    async def permission_middleware(cls, request, handler):
        if request.session is not None:
            # Check logged-in user role has permission for requested operation
            required = await cls._get_roles(request)
            verdict = (request.session.role & required) == required
            Log.debug(f'Required user role: {required} for operation')
            Log.debug(f'User role: {request.session.role}')
